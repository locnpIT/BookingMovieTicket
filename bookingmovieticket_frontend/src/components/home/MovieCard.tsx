...
export default function MovieCard({ movie, className }: { movie: Movie; className?: string }) {
  return (
    <div className={`group relative shrink-0 snap-start overflow-hidden rounded-xl bg-white/95 shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg ${className ?? 'w-56'}`}>
      <div className="relative h-80 w-full overflow-hidden">
        ...
        <div className="absolute bottom-2 left-2 right-2 hidden items-center justify-between gap-2 group-hover:flex">
          <button
            className="flex-1 rounded-lg bg-gradient-to-r from-sky-600 to-indigo-600 px-4 py-2 text-sm font-medium text-white shadow hover:from-sky-700 hover:to-indigo-700 cursor-pointer"
            aria-label="Đặt vé"
          >
            Đặt vé
          </button>
          <button
            className="flex-1 rounded-lg bg-orange-500 px-4 py-2 text-sm font-medium text-white shadow hover:bg-orange-600 cursor-pointer"
