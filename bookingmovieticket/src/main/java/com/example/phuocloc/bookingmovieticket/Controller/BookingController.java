...
public class BookingController {
    ...
    public ResponseEntity<ApiResponse<BookingDTO>> confirm(@Valid @RequestBody BookingConfirmRequest request, Authentication auth) {
        ...
        BookingDTO dto = bookingFlowService.confirm(user, request.getShowSeatIds(), BigDecimal.ZERO);
    ...
    @GetMapping("/me")
    @PreAuthorize("hasAnyRole('USER','ADMIN')")
    public ResponseEntity<ApiResponse<java.util.List<BookingDTO>>> myBookings(Authentication auth) {
        User user = ((CustomUserDetails) auth.getPrincipal()).getUser();
        java.util.List<BookingDTO> items = bookingFlowService.listBookings(user);
        ApiResponse<java.util.List<BookingDTO>> res = new ApiResponse<>(true, "Lấy lịch sử đặt vé thành công", items, HttpStatus.OK.value(), LocalDateTime.now());
